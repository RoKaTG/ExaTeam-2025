...
FROM nexus.retd.edf.fr:5012/debian:buster-slim
...
RUN \
    CMAKE_SRC=https://minio.retd.edf.fr/codeaster/containers-data/prerequisites ; \
    CMAKE_URL=${CMAKE_SRC}/cmake-3.24.1-linux-x86_64.tar.gz ; \
    curl --insecure -L -o cmake.tar.gz "${CMAKE_URL}" ; \
    mkdir -p /opt/cmake ; \
...
RUN \
    export PIP_OPTIONS="--trusted-host nexus.retd.edf.fr --index-url https://nexus.retd.edf.fr/repository/pypi-all/simple" ; \
