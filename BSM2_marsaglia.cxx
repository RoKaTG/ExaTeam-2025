/* 
    ...
    Compilation : g++ -O3 -ffast-math -funroll-loops -march=native -fopenmp BSM2_marsaglia.cxx -o BSM2_marsaglia
...
*/
// Naive implementation of the Marsaglia Polar Method
// Converges correctly but vey slow
double marsaglia_polar_method() {
    std::random_device rd;
    std::mt19937 gen(rd());
    double u, v, s;
    do {
        u = 2.0 * (gen() / static_cast<double>(gen.max())) - 1.0;
        v = 2.0 * (gen() / static_cast<double>(gen.max())) - 1.0;
        s = (u*u) + (v*v);
    } while (s >= 1.0 || s == 0.0);
...
double black_scholes_monte_carlo(ui64 S0, ui64 K, double T, double r, double sigma, double q, ui64 num_simulations) {
    ...
    for (ui64 i = 0; i < num_simulations; ++i) {
        double Z = marsaglia_polar_method();
